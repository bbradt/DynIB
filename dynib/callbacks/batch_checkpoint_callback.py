...
Checkpoint = namedtuple("BatchCheckpoint", field_names=["obj", "logpath", "metric"])


class BatchCheckpointCallback(ICheckpointCallback):
    ...
    def __init__(
        ...
        if minimize is not None:
            ...
            self.on_batch_end = self.on_epoch_end_best
        else:
            self._minimize = False
            self.on_epoch_end = self.on_epoch_end_last
            self.on_batch_end = self.on_epoch_end_last
    ...
    def _handle_epoch(self, runner: "IRunner", score: float):
        ...
        logprefix = f"{self.logdir}/epoch{runner.epoch_step:04d}/{self.mode}.e{runner.epoch_step:04d}.{runner.batch_step:06d}"
        os.makedirs(f"{self.logdir}/epoch{runner.epoch_step:04d}", exist_ok=True)
        logpath = self._save(runner, obj, logprefix)
