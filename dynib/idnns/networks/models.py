...
from dynib.idnns.networks.ops import *


def multi_layer_perceptron(x, n_input, num_classes, hidden_layer_sizes_1, hidden_layer_sizes_2):
    hidden = []
    input = []
    hidden.append(x)
    # Network Parameters
    ...
    weights = {
        'h1': tf.Variable(tf.random_normal([n_input, hidden_layer_sizes_1])),
        'h2': tf.Variable(tf.random_normal([hidden_layer_sizes_1, hidden_layer_sizes_2])),
        'out': tf.Variable(tf.random_normal([hidden_layer_sizes_2, num_classes]))
    }
    biases = {
        'b1': tf.Variable(tf.random_normal([hidden_layer_sizes_1])),
        'b2': tf.Variable(tf.random_normal([hidden_layer_sizes_2])),
